...
public:
    ...
    bool find_bfs(vector<BFS_Node_126*>& all_levels,int beg, int end, int depth, list<string>& wordList, const string&endWord, vector<vector<string>>&result)
    {
        if (beg >= end)
        {
            return false;
        ...
        for (int i = beg; i < end; ++i)
        {
            for (int used_i = 0; used_i < used_data.size(); ++used_i)
            {   //data used than once ,in same level
                if (!is_one_diff(used_data[used_i]->val, all_levels[i]->val))
                {
                    continue;
                }
                used_data[used_i]->parents.push_back(all_levels[i]);
            }
            list<string>::iterator word_it = wordList.begin();
            while (word_it != wordList.end())
            {
                if (!is_one_diff(all_levels[i]->val, *word_it))
                {
                    word_it++;
                    continue;
                }
                ...
                if (*word_it == endWord)
                {
                    vector<string> one_result(depth + 1);
                    one_result[depth] = endWord;
                    get_result(all_levels[i], depth - 1, one_result, result);
                    find_end = true;
                    break;
                }
                else {
                    BFS_Node_126* son_node = new BFS_Node_126();
                    son_node->val = *word_it;
                    son_node->parents.push_back(all_levels[i]);

                    all_levels.push_back(son_node);
        ...
        return find_bfs(all_levels,end,all_levels.size(), depth + 1, wordList, endWord, result);
    }
    void clear_children(vector<BFS_Node_126*>& node_group)
    {
    ...
    vector<vector<string>> findLadders(string beginWord, string endWord, vector<string>& wordList) {
        ...
        vector<BFS_Node_126*> levels;
        levels.push_back(root_node);


        list<string> word_list;
        std::copy(wordList.begin(), wordList.end(), back_inserter(word_list));
        find_bfs(levels,0,levels.size(), 1, word_list, endWord, result);

        clear_children(levels);
        return result;
    }
};

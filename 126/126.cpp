...
public:
    ...
    void get_result(BFS_Node_126* bfs_node, int pos, vector<string>&data)
    {
        if (bfs_node->parent == NULL)
        {
            return ;
        }
        data[pos] = bfs_node->val;
        get_result(bfs_node->parent, pos - 1, data);
    }
    bool find_bfs(vector<BFS_Node_126*>& fathers, int depth, list<string>& wordList, const string&endWord, vector<vector<string>>&result)
    {
        if (wordList.empty())
        {
            return false;
        ...
        {
            for (int used_i = 0; used_i < used_data.size(); ++used_i)
            {   //data used than once ,in same level
                if (!is_one_diff(used_data[used_i], fathers[i]->val))
                {
                    continue;
                }
                BFS_Node_126* son_node = new BFS_Node_126();
                son_node->val = used_data[used_i];
                son_node->parent = fathers[i];
                fathers[i]->childrens.push_back(son_node);
                childrens.push_back(son_node);
            }
            list<string>::iterator word_it = wordList.begin();
            while (word_it != wordList.end())
            {
                if (!is_one_diff(fathers[i]->val, *word_it))
                {
                    word_it++;
                    continue;
                }
                ...
                if (*word_it == endWord)
                {
                    vector<string> one_result(depth + 1);
                    one_result[depth] = endWord;
                    get_result(fathers[i], depth - 1, one_result);
                    ...
                    BFS_Node_126* son_node = new BFS_Node_126();
                    son_node->val = *word_it;
                    son_node->parent = fathers[i];
                    fathers[i]->childrens.push_back(son_node);
                    childrens.push_back(son_node);
                    used_data.push_back(*word_it);
                    wordList.erase(word_it++);
                    continue;
                }
            }
        ...
        return find_bfs(childrens, depth + 1, wordList, endWord, result);
    }
    void clear_node(BFS_Node_126*root_node)
    {
        if (root_node == NULL)
        {
            return ;
        }
        for (int i = 0; i < root_node->childrens.size(); ++i)
        {
            clear_node(root_node->childrens[i]);
        }
        delete root_node;
    }
    ...
    vector<vector<string>> findLadders(string beginWord, string endWord, vector<string>& wordList) {
        ...
        BFS_Node_126* root_node = new BFS_Node_126();
        root_node->val = beginWord;

        BFS_Node_126* son_node = new BFS_Node_126();
        son_node->val = beginWord;
        son_node->parent = root_node;
        root_node->childrens.push_back(son_node);

        list<string> word_list;
        std::copy(wordList.begin(), wordList.end(), back_inserter(word_list));
        find_bfs(root_node->childrens, 1, word_list, endWord, result);
...
public:
    ...
    void get_result(BFS_Node_126* bfs_node, int pos, vector<string>&data, vector<vector<string>>&result)
    {
        if (bfs_node->parents.empty())
        {
            result.push_back(data);
            return ;
        }
        data[pos] = bfs_node->val;
        for (int i = 0; i < bfs_node->parents.size(); ++i)
        {
            get_result(bfs_node->parents[i], pos - 1, data, result);
        }
    }
    bool find_bfs(vector<BFS_Node_126*>& fathers, int depth, list<string>& wordList, const string&endWord, vector<vector<string>>&result)
    {
        ...
        bool find_end = false;
        vector<BFS_Node_126*> childrens;
        vector<BFS_Node_126*> used_data;

        for (int i = 0; i < fathers.size(); ++i)
        {
            for (int used_i = 0; used_i < used_data.size(); ++used_i)
            {   //data used than once ,in same level
                if (!is_one_diff(used_data[used_i]->val, fathers[i]->val))
                {
                    continue;
                }

                if (used_data[used_i]->val == "rapid")
                {
                    printf("count\n");
                }
                fathers[i]->childrens.push_back(used_data[used_i]);
                used_data[used_i]->parents.push_back(fathers[i]);
            }
            list<string>::iterator word_it = wordList.begin();
            while (word_it != wordList.end())
            {
                ...
                if (*word_it == endWord)
                {
                    vector<string> one_result(depth + 1);
                    one_result[depth] = endWord;
                    get_result(fathers[i], depth - 1, one_result, result);
                    find_end = true;
                    break;
                }
                else {
                    BFS_Node_126* son_node = new BFS_Node_126();
                    son_node->val = *word_it;
                    son_node->parents.push_back(fathers[i]);
                    fathers[i]->childrens.push_back(son_node);
                    childrens.push_back(son_node);
                    used_data.push_back(son_node);
                    wordList.erase(word_it++);


                    if (son_node->val  == "rapid")
                    {
                        printf("count\n");
                    }
                    continue;
    ...
    void clear_children(BFS_Node_126*root_node)
    {
        if (root_node->childrens.empty())
        {
            return ;
        }
        ...
        for (int i = 0; i < root_node->childrens.size(); ++i)
        {
            if(root_node->childrens[i]->val=="rapid")
            {
                cout<<"info:"<<root_node->childrens.size();
                for(int x=0;x<root_node->childrens.size();++x)
                {
                    cout<<","<<root_node->childrens[x]->val;
                }
                cout<<endl;
                cout<<"parents:"<<root_node->parents.size();
                for(int x=0;x<root_node->parents.size();++x)
                {
                    cout<<","<<root_node->parents[x]->val;
                }
                cout<<endl;
            }
            clear_children(root_node->childrens[i]);
            
            if (root_node->childrens[i]->parents.empty())
            {
                cout <<root_node->val<<":"<< root_node->childrens[i]->val << endl;
                cout << "errrrrrrrrrrrrrr" << endl;
                exit(0);
            }
            root_node->childrens[i]->parents.pop_back();
            if (root_node->childrens[i]->parents.empty())
            {
                delete root_node->childrens[i];
            }
        }
    }
    ...
    vector<vector<string>> findLadders(string beginWord, string endWord, vector<string>& wordList) {
        ...
        BFS_Node_126* son_node = new BFS_Node_126();
        son_node->val = beginWord;
        son_node->parents.push_back(root_node);
        root_node->childrens.push_back(son_node);
